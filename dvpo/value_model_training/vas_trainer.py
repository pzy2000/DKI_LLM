...
class VASTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        validation_dataset: Optional[Union[torch.utils.data.Dataset, Dataset]] = None,
        optimizer: Optional[torch.optim.Optimizer] = None,
        data_collator: Optional[typing.Callable] = None,
        validation_data_collator: Optional[typing.Callable] = None,
        ...
        if not config.train_value_only:
        ...
        if self.validation_dataset is not None:
            self.validation_dataloader = self.prepare_dataloader(
                self.validation_dataset, validation_data_collator
            )
        else:
    ...
    def step(
        ...
        with torch.no_grad():
            ...
            #print('advantage_values')
            #print(values)
            #print('advantage_returns')
            #print(returns)
    ...
    def evaluate(self):
        """
        在验证集上评估模型，并打印验证集上的平均损失。
        """
        if self.validation_dataloader is None:
            raise ValueError("No validation dataloader found. Please provide a validation dataset.")
        ...
        with torch.no_grad():
            for batch in self.validation_dataloader:
                #print(batch)
                # 假设批次包含 'queries', 'responses', 'scores', 和可选的 'response_masks'
                queries = batch.get('query')
                responses = batch.get('response')
                scores = batch.get('rewards')
                response_masks = batch.get('response_masks', None)

                if queries is None or responses is None or scores is None:
                    raise ValueError("Batch must contain 'queries', 'responses', and 'scores'.")

                # 使用 _step_safety_checker 进行安全检查和处理
                bs = self.config.batch_size
                queries, responses, scores, response_masks = self._step_safety_checker(
                    bs, queries, responses, scores, response_masks
                )
