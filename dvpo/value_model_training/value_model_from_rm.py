...
def build_response_train_dataset(config, dataset_name):
    ...
    def tokenize(sample):
        sample["text"] = sample["value_question"] + sample["value_response"]
        #print(sample['text'])
        sample["query"] = tokenizer.encode(sample["value_question"])[:-1]
        sample["response"] = tokenizer.encode(sample["value_response"])[1:]
    ...
    ds = ds.filter(lambda sample: len(sample["response"]) > 0)
    ds = ds.filter(lambda sample: (len(sample["response"])+len(sample["query"])) < args.max_length)
    print(len(ds))
