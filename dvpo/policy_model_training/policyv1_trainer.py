...
from trl_policy.import_utils import is_torch_greater_2_0
...
class Policyv1Trainer(BaseTrainer):
    """
    ...
    _tag_names = ["trl", "policyv1"]

    def __init__(
            self,
            config: Optional[Policyv1Config] = None,
            model: Optional[PreTrainedModelWrapper] = None,
            ref_model: Optional[PreTrainedModelWrapper] = None,
            value_model: Optional[PreTrainedModelWrapper] = None,
        ...
        Args:
            ...
            model (`PreTrainedModelWrapper`):
                Hugging Face transformer model with h a casual language modelling head. Used for update policy.
            ref_model (`PreTrainedModelWrapper`):
                Hugging Face transformer model with a casual language modelling head. Used for KL penalty
            value_model (`PreTrainedModelWrapper`):
        ...
        if not isinstance(config, Policyv1Config):
            raise ValueError(f"config must be a PolicyConfig, got {type(config)}")
        ...
        if isinstance(value_model, SUPPORTED_ARCHITECTURES):
            self.value_model = value_model
        else:
            raise ValueError(
                f"value_model must be a PreTrainedModelWrapper with value head, got {type(value_model)} - supported "
                f"architectures are: {SUPPORTED_ARCHITECTURES} "
            )
    ...
    def _step_safety_checker(
        ...
        return queries, responses,  masks
    ...
    def step(
        ...
        queries, responses,  response_masks = self._step_safety_checker(
            bs, queries, responses,  response_masks
        )
        ...
        with torch.no_grad():
            ...
            if full_kl_penalty:
                ...
                non_score_reward, kls = self.compute_rewards(
                     active_full_logprobs, ref_full_logprobs, masks
                )
            else:
                non_score_reward, kls = self.compute_rewards(all_logprobs, ref_logprobs, masks)
            timing["time/ppo/compute_rewards"] = time.time() - t

            t = time.time()
            values, advantages, returns = self.compute_advantages(values,non_score_reward, masks)
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            logits = (model(**input_kwargs))['logits']
            #print(model(**input_kwargs))

            value_model.eval()
            with torch.no_grad():
                _,_,values = value_model(**input_kwargs)
                #print(value_model(**input_kwargs))
    ...
    def masked_percentile(self,values: torch.Tensor, mask: torch.Tensor, percent: float, axis: Optional[int] = None) -> torch.Tensor:
        """
        Compute the percentile of values with a mask.

        Args:
            values (torch.Tensor): The input tensor, shape (batch_size, response_length).
            mask (torch.Tensor): A mask tensor, same shape as `values`, with 1 for valid values and 0 for invalid.
            percent (float): The percentile to compute (0-100).
            axis (Optional[int]): The axis along which to compute the percentile. Default is None.

        Returns:
            torch.Tensor: The computed percentile.
        """
        if axis is None:
            # Flatten and mask values
            valid_values = values[mask.bool()]
            if valid_values.numel() == 0:
                return torch.tensor(float('nan'))  # Return NaN if no valid values
            return torch.quantile(valid_values, percent / 100.0)

        else:
            # Mask values along the specified axis
            masked_values = torch.where(mask.bool(), values, torch.tensor(float('nan'), device=values.device))
            percentiles = torch.nanquantile(masked_values, percent / 100.0, dim=axis)
            return percentiles
    ...
    def compute_rewards(
        ...
        return  torch.stack(non_score_rewards), torch.stack(kls)
    ...
    '''def generate_value(self,**input_kwargs):
        self.value_model.eval()
        with torch.no_grad():
            _,_,values = self.value_model(**input_kwargs)
            values = whiten(values)
            values = values.detach()
        return values'''
    ...
    def loss(
        ...
        p1 = self.masked_percentile(values,mask,1)
        p3 = self.masked_percentile(values,mask,3)
        p5 = self.masked_percentile(values,mask,5)
        p7 = self.masked_percentile(values,mask,7)
        p9 = self.masked_percentile(values,mask,9)

        p93 = self.masked_percentile(values,mask,93)
        p97 = self.masked_percentile(values,mask,97)
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        stats = {
            ...
            "policy/mean_non_score_reward": mean_non_score_reward,
            "policy/mean_value_mask":mean_mask_value
        ...
        for k, v in data["train_stats"].items():
            stats[f"policy/{k}"] = torch.mean(v, axis=0)
    ...
    def log_stats(
        ...
        if self.accelerator.is_main_process:
            ...
            if "query" not in batch.keys() and "response" not in batch.keys():
                ...
                logs.update({"game_log": wandb.Table(columns=[*columns_to_log], rows=table_rows)})'''
